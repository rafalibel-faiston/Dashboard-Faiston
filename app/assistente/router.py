...
# O piloto acabou: o assistente é feature de todo mundo que tem sessão, sem
# lista de perfis nem env var pra manter (era ASSISTENTE_PERFIS_PILOTO).
# Estar logado é o único requisito pra perguntar; gerenciar a base de
# procedimentos continua só admin, via _exigir_admin.
...
async def _exigir_admin(faiston_token: Optional[str]) -> dict:
    """Gestão de conteúdo é mais sensível que só perguntar: exige
    perfil='admin' de verdade, mesmo com o assistente aberto a todos os
    perfis."""
    sess = await run_in_threadpool(db.get_session, faiston_token)
    if not sess:
        raise HTTPException(status_code=401, detail="Não autenticado")
...
# outros endpoints de gestão, nunca exposto aos demais perfis.
